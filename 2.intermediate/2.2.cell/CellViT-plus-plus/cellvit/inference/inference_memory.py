...
class CellViTInferenceMemory(CellViTInference):
    ...
    def process_wsi(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            dataloader_iter = iter(wsi_inference_dataloader)
            pbar = tqdm.tqdm(total=len(wsi_inference_dataloader))
            ...
            while True:
                try:
                    ...
                    self.logger.warning(f"Skipping batch {batch_num} due to IndexError: {e}")
                    batch_num += 1
                    pbar.update(1)
                    continue
                ...
                if patches.nelement() == 0:
                    print(f"Skipping batch {batch_num} because patches is empty.")
                    pbar.update(1)
                    continue
