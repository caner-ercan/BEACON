...
class ExperimentCellVitClassifier(BaseExperiment):
    ...
    def run_experiment(self) -> tuple[Path, dict, nn.Module, dict]:
        ...
        if not os.path.exists(self.run_conf["logging"]["log_dir"]):
            self.store_config()
        # deneme =  self.run_conf.get('cellvit_backbone', "olmadi")
        ...
        train_transforms, val_transforms = self.get_transforms(
            ...
            normalize_settings_default=cellvit_run_conf["transformations"].get("normalize",None),
        ...
        # Call fit method
        if 'skip_train' not in self.run_conf:
        ...
        # self.save_preds(self, train_dataloader, val_dataloader, test_dataloader, save_cm = True)
    ...
    def get_datasets(
        ...
        test_filelist: Union[Path, str] = None,
    ...
    def load_cellvit_model(
        ...
        arch = self.run_conf.get('cellvit_backbone', None)
        if arch is None:
            arch = cellvit_run_conf['model']['arch'].title()
        # self.logger.info(f"Arch of backbone: {arch}")
        cellvit_run_conf["model"]["backbone"]=arch
        ...
        # cellvit_run_conf["model"]["backbone"]=cellvit_run_conf['model']['arch'].title()
    ...
    def _get_cellvit_architecture(
        ...
        if model_type in ["CellViT"]:
            ...
            model = CellViTSAM(
                ...
                num_nuclei_classes=model_conf["data"]["num_nuclei_classes"],
                num_tissue_classes=model_conf["data"]["num_tissue_classes"],
                vit_structure=model_conf["model"].get("backbone","SAM-H"),
                regression_loss=model_conf["model"].get("regression_loss", False),
            )
    ...
    #     checkpoint_path = Path(self.run_conf["logging"]["log_dir"]) / "checkpoints" / "model_best.pth"
