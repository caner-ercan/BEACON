...
class CellViTInference:
    ...
    def __init__(
        ...
        self.label_map: dict = None #= TYPE_NUCLEI_DICT_PANNUKE
        ...
        if self.binary:
            ...
            self.label_map = self.run_conf["data"].get("label_map", TYPE_NUCLEI_DICT_PANNUKE) 
            self.label_map = {int(key): value for key, value in self.label_map.items()}
        self.logger.info(f"Cell class dict: {self.label_map}")
    ...
    def _load_model(self) -> None:
        ...
        # if not self.run_conf['model']['backbone']:
        #    self.run_conf['model']['backbone'] = self.backbone
        # print(self.run_conf)
        # print(self.run_conf["transformations"])
        ...
        if "dataset_config" not in self.run_conf or self.run_conf["dataset_config"] is None:
            self.run_conf["dataset_config"] = {}

        if "nuclei_types" not in self.run_conf["dataset_config"] or not self.run_conf["dataset_config"]["nuclei_types"]:
            self.run_conf["dataset_config"]["nuclei_types"] = {"Background": 0, "nucleus":1}
    ...
    def _load_classifier(self, classifier_path: Union[Path, str] = None) -> None:
        ...
        if classifier_path is None:
            ...
            classifier_model_checkpoint = torch.load(classifier_path, map_location="cpu", weights_only = False)
            self.logger.info(f"Classifer loaded from: {classifier_path}")
            classifier_run_conf = unflatten_dict(classifier_model_checkpoint["config"], ".")

            # self.logger.info(classifier_run_conf)
            ...
            self.logger.info(
                classifier_model.load_state_dict(classifier_model_checkpoint["model_state_dict"])
            )
            # model = model  # .to(self.device)
            classifier_model.eval()
            self.label_map = classifier_run_conf["data"]["label_map"]
    ...
    def _get_model(
        ...
        if model_type in ["CellViT"]:
            ...
            model = CellViTSAM(
                ...
                num_nuclei_classes=self.run_conf["data"]["num_nuclei_classes"],
                num_tissue_classes=self.run_conf["data"]["num_tissue_classes"],
                vit_structure=self.run_conf['model'].get('backbone', self.backbone),
                regression_loss=self.run_conf["model"].get("regression_loss", False),
            )
    ...
    def _convert_json_geojson(
        ...
        if polygons:
            ...
            detected_types = sorted(map(int, cell_segmentation_df.type.unique()))
            geojson_placeholder = []
            ...
            detected_types = sorted(map(int, cell_detection_df.type.unique()))
            geojson_placeholder = []
            for cell_type in detected_types:
