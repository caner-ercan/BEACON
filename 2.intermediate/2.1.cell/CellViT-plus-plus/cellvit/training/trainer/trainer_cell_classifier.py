...
class CellViTHeadTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            cellvit_run_conf=cellvit_run_conf
        )
        self.num_classes = num_classes
        self.cellvit_model = cellvit_model
        self.cellvit_model.eval()
        self.cellvit_run_conf=cellvit_run_conf
        ...
        if self.num_classes <= 2:
            ...
            self.mca = MulticlassAccuracy(num_classes=self.num_classes, average=None)
            self.cm = MulticlassConfusionMatrix(num_classes=self.num_classes)
            self.mcf1s = MulticlassF1Score(num_classes=self.num_classes, average=None)
        ...
        # self.unfreeze_epoch = experiment_config["training"].get("unfreeze_epoch", 0)
    ...
    def _cache_results(self, extracted_cells: List, dataset_part: str) -> None:
        ...
        if dataset_part == "train":
            ...
            cache_path = dataset_path / "cache" / f"{self.test_dataset_hash}.h5"
            self.logger.info(f"Caching dataset {self.test_dataset_hash} to disk...")
    ...
    def fit(
        ...
        **kwargs,
        ...
        self._calculate_hashes(train_dataloader, val_dataloader, test_dataloader)
                    ...
                    if best_model:
                        ...
                        if test_dataloader is not None:
                            ...
                            (
                                ...
                                _,
                                test_results,
                            ) = self.validation_epoch(epoch, test_dataloader, "test")
                            wandb.log(test_scalar_metrics, step=epoch+1)
                            val_scalar_metrics.update(test_scalar_metrics)
                            # self.logger.info(val_scalar_metrics)
                        ...
                        self.logger.info("Test dataset metrics:")
                        self.logger.info(test_scalar_metrics)
                        break
            if self.early_stopping is None:        
                self.save_checkpoint(epoch, "latest_checkpoint.pth")
            else:
                self.model = self.best_model
            self.save_preds(self, train_dataloader, val_dataloader, test_dataloader,epoch )
    ...
    def train_epoch(
        ...
        postprocessor = DetectionCellPostProcessorCupy(wsi=None, nr_types=self.cellvit_run_conf["data"]["num_nuclei_classes"])#, nr_types=6)
        ...
        avg_loss = float(self.loss_avg_tracker.avg.detach().cpu().numpy())
        scalar_metrics = {
            "Loss/Train": avg_loss,
        ...
        # class specific metrics added to scalar metrics
        if self.num_classes >2:
            class_specific_metrics = self.calculate_class_specific_metrics(
                probabilities, gt, self.experiment_config['data']['label_map']
            )
            scalar_metrics.update(class_specific_metrics)
    ...
    def validation_epoch(
        ...
        # inference using cellvit
        if epoch == 0:
            dataset_cache_exists = self._test_cache_exists(mode)
            if dataset_cache_exists:
                extracted_cells = self._load_from_cache(mode)
            else:
                self.logger.info(f"Extracting {mode} cells")
                with torch.no_grad():
                    for idx, (
                        images,
                        cell_gt_batch,
                ...
                self._cache_results(extracted_cells, mode)
            self.cached_dataset[f"{mode.capitalize()}-Cells"] = extracted_cells
        else:
            extracted_cells = self.cached_dataset[f"{mode.capitalize()}-Cells"]
        ...
        #TODO_CE move detailed metrics to a separate function to run only on the best model after training
        ...
        avg_loss = float(self.loss_avg_tracker.avg.detach().cpu().numpy())
        scalar_metrics = {
            f"Loss/{mode}": avg_loss,
            f"F1-Score/{mode}/overall": f1_score,
            f"Accuracy-Score/{mode}/overall": acc_score,
            f"AUROC/{mode}": auroc_score,
            f"Average-Precision/{mode}": average_prec_score,
        }
    ...
    def store_best_val_results(self, val_results: dict, dataset = "val") -> None:
        """Store best validation results (checkpoint)

        Args:
            val_results (dict): Validation results
        """
        val_result_dir = self.logdir / f"{dataset}_results"
    ...
    def store_best_val_scores(self, val_scores: dict) -> None:
        ...
        self.logger.info(print(val_scores))
        with open(filename, "w") as json_file:
            json.dump(val_scores, json_file, indent=2)

    def calculate_class_specific_metrics(self, probabilities, gt, label_map, mode = "val"):
            """Calculate class-specific metrics.

            Args:
                probabilities (torch.Tensor): Predicted probabilities.
                gt (torch.Tensor): Ground truth labels.
                label_map (dict): Mapping of class indices to class names.

            Returns:
                dict: Dictionary containing class-specific metrics.
            """
            class_specific_metrics = {}

            mca_scores = self.mca(probabilities, gt)
            # mca_scores = mca_scores.cpu().numpy()
            # print(mca_scores)
            mca_scores = np.float32(mca_scores)

            # cm = self.cm(probabilities, gt)
            # print(cm)
            # cm = np.int32(cm)
            ...
            # cm = cm.cpu().numpy()
            # class_specific_metrics["confusion_matrix"] = cm

            f1_scores = self.mcf1s(probabilities, gt)
            f1_scores = np.float32(f1_scores)
            # print(f1_scores)
            ...
            for class_idx, class_name in label_map.items():
                ...
                class_acc_score = mca_scores[class_idx]
                class_f1_score = f1_scores[class_idx]
                class_specific_metrics[f"F1-Score/{mode}/{class_name}"] = class_f1_score
                class_specific_metrics[f"Accuracy-Score/{mode}/{class_name}"] = class_acc_score

            return class_specific_metrics


    def save_preds(self, train_dataloader, val_dataloader, test_dataloader,epoch, save_cm = True):
        (_,_,_,val_results,) = self.validation_epoch(epoch, val_dataloader, "val")
        (_,_,_,test_results,) = self.validation_epoch(epoch, test_dataloader, "test")
        (_,_,_,train_results,) = self.validation_epoch(epoch, train_dataloader, "Train")
        self.store_best_val_results(train_results,"train")
        self.store_best_val_results(val_results,"val")
        self.store_best_val_results(test_results,"test")
        ...
        #merge all predictions
            all_predictions = []
            all_gt = []
            all_predictions.append(train_results["predictions"])
            all_gt.append(train_results["gt"])
            all_predictions.append(val_results["predictions"])
            all_gt.append(val_results["gt"])
            all_predictions.append(test_results["predictions"])
            all_gt.append(test_results["gt"])
            all_predictions = torch.cat(all_predictions, dim=0)
            all_gt = torch.cat(all_gt, dim=0)

            labels = [v[:5] for k, v in self.experiment_config['data']['label_map'].items()]
            self.plot_cm(train_results["predictions"], train_results["gt"], "train", labels)
            self.plot_cm(val_results["predictions"], val_results["gt"], "val", labels)
            self.plot_cm(test_results["predictions"], test_results["gt"], "test", labels)
            self.plot_cm(all_predictions, all_gt, "total", labels)

    def plot_cm(self,predictions, gt, dataset_name,labels):
        from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay
        import matplotlib.pyplot as plt
        cm_folder = self.logdir / "cm"
        cm_folder.mkdir(exist_ok=True, parents=True)
        filename=str(cm_folder / f"cm_{dataset_name}.png")
