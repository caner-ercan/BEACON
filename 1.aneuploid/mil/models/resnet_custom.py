...
def resnet50_baseline(arch = "resnet50", pretrained=False):
    ...
    if arch == "resnet50":
        ...
        # to get 1024-dimensional feature vector,
        # Remove the fully connected layer and the softmax layer
        #model = torch.nn.Sequential(*list(model.children())[:-1])
        # to get a fixed-size output regardless of input size
        #model.add_module('global_avg_pool', nn.AdaptiveAvgPool2d(1024))
        #in_features = model.classifier.in_features
        model.classifier = nn.Linear(model.classifier.in_features, 1024, bias=True)
...
# def load_pretrained_weights(model, name):
#    # I added this part to use local model
...
local_weights_path = '/rsrch5/home/trans_mol_path/cercan/code/Barretts/CLAM/models/resnet50-19c8e357.pth'
