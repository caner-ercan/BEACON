...
parser.add_argument('--k', type=int, default=1,
                    help='number of train/val/test splits to generate (default: 1, single fixed split)')
...
if __name__ == '__main__':
    ...
    if args.test_frac > 0: 
        for lf in label_fracs:
            split_dir = os.path.join(args.split_path,str(args.task) + '_{}'.format(int(lf * 100)))
        ...
        for lf in label_fracs:
            split_dir = os.path.join(args.split_path,str(args.task) + '_{}'.format(int(lf * 100)))
            os.makedirs(split_dir, exist_ok=True)
            dataset.create_splits_notest(k = args.k, val_num = val_num, label_frac=lf)
            len(dataset)
            for i in range(args.k):
                dataset.set_splits_notest()
                descriptor_df = dataset.test_split_gen_notest(return_descriptor=True)
                dataset.test_ids=[]
                splits = dataset.return_splits(from_id=True)
