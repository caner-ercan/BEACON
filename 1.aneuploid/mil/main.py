...
def main(args):
    ...
    # Single fixed train/val/test split, identified by args.split (matches a checkpoint file
    # named s_{split}_checkpoint.pt) — one training run, no fold sweep.
    i = args.split
    ...
    if not os.path.isfile(filename_pkl):
        ...
        summary_df = pd.DataFrame({'split': [i], 'int_test_auc': [test_auc], 'val_auc': [val_auc],
                                 'int_test_f1_score': [test_f1_score], 'val_f1_score': [val_f1_score],
                                 'int_test_acc': [test_acc], 'int_test_specificity': [test_accs[0]], 'int_test_sensitivity': [test_accs[1]],
                                 'val_acc': [val_acc], 'val_specificity': [val_accs[0]], 'val_sensitivity': [val_accs[1]],
                                 'int_test_balanced_acc': [np.mean(test_accs)], 'val_balanced_acc': [np.mean(val_accs)]})
        summary_df = summary_df.round(3)
        summary_df.to_csv(os.path.join(args.results_dir, 'summary.csv'))
...
parser.add_argument('--split', type=int, default=0, help='index of the single train/val/test split to train on (default: 0)')
