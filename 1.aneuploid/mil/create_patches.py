...
def stitching(file_path, downscale = 2, rgb_wsi = None):
	start = time.time()
	heatmap = StitchPatches(file_path, downscale=downscale, bg_color=(0,0,0), alpha=-1, draw_grid=False, original_image = rgb_wsi)
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
	...
	for i in range(total):
		...
		if stitch:
			...
			stitch_downscale = 2
			rgb_wsi = WSI_object.getRGBImage(downscale=stitch_downscale)
			heatmap, stitch_time_elapsed = stitching(file_path, downscale=stitch_downscale, rgb_wsi = rgb_wsi)
