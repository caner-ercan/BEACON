...
def parse_config_dict(args, config_dict):
	...
	config_dict['args']['aggregate']=args.aggregate
...
def collect_probabilities_df(asset_dict, patch_size=(224, 224), stride_factor=0.5):
	...
	avg_df = df.groupby('coords').agg(
		attention_scores=('attention_scores', 'mean'),
		A_raw=('A_raw', 'mean'),
		attention_scores_minmax=('attention_scores_minmax', 'mean'),
		A_raw_minmax=('A_raw_minmax', 'mean')
	).reset_index()

	avg_attention_scores_minmax = min_max_normalization(avg_df['attention_scores'])
	avg_df['avg_attention_scores_minmax'] = avg_attention_scores_minmax
	...
	avg_asset_dict = { 'coords': np.array(avg_df['coords'].tolist(), dtype=int), 
		...
		'A_raw':np.array(avg_df['A_raw'].tolist(), dtype=float), 
		'attention_scores_minmax':np.array(avg_df['attention_scores_minmax'].tolist(), dtype=float), 
		'A_raw_minmax':np.array(avg_df['A_raw_minmax'].tolist(), dtype=float), 
		'avg_attention_scores_minmax': avg_attention_scores_minmax 
		}
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		if os.path.exists(r_slide_save_dir):
			print('skipped: '+r_slide_save_dir)
			continue
		else:
			...
			if os.path.exists(block_map_save_path):
				...
				asset_dict['A_raw'] = np.array(A_raw.cpu()).astype(np.float32) 

				A_softmax = F.softmax(A_raw, dim=0)
				A_softmax = np.array(A_softmax)
				asset_dict['attention_scores'] = A_softmax

				A_raw_minmax = min_max_normalization(A_raw.cpu().numpy())
				...
				if args.aggregate:
					asset_dict =collect_probabilities_df(asset_dict)



				A_raw_minmax_binary = (A_raw_minmax > 0.5).astype(int)
				A_softmax_minmax_binary = (A_softmax_minmax > 0.5).astype(int)
				asset_dict['A_raw_minmax_binary'] = A_raw_minmax_binary
				asset_dict['A_softmax_minmax_binary'] = A_softmax_minmax_binary
				# print(asset_dict)
			...
			if not os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
				coord_dset = asset_dict['coords'][:]
				# avg_attention_scores = asset_dict['attention_scores']
				# avg_raw_attention_score = asset_dict['raw_attention_scores']
				# avg_attention_scores_minmax = asset_dict['attention_scores_minmax']
				# avg_raw_attention_scores_minmax = asset_dict['raw_attention_scores_minmax']
				...
				# Attention heatmap for every key in asset_dict
				for key in asset_dict.keys():
					heatmap = drawHeatmap(asset_dict[key], coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
								thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=True, map_any =False)
					heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format(key,slide_id,patch_size[0])))
				# heatmap = drawHeatmap(A_raw, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=True, map_any =False)
			
				# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format('A_raw',slide_id,patch_size[0])))


				# heatmap = drawHeatmap(A_raw_minmax, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				...
				# heatmap = drawHeatmap(A_raw_minmax_binary, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False)
			
				# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format('A_raw_minmax_binary',slide_id,patch_size[0])))


				# heatmap = drawHeatmap(A_softmax_minmax, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False)
			
				# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format('A_softmax_minmax',slide_id,patch_size[0])))

				# heatmap = drawHeatmap(A_softmax_minmax_binary, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False)
			
				# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format('A_softmax_minmax_binary',slide_id,patch_size[0])))


				# heatmap = drawHeatmap(A_softmax, coord_dset, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
				# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False)
			
				# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_{}px_blockmap.png'.format('A_softmax',slide_id,patch_size[0])))
				del heatmap
