...
def infer_single_slide(model, features, label, reverse_label_dict, k=1,tile_prob= False):
	...
	with torch.no_grad():
		if isinstance(model, (CLAM_SB, CLAM_MB)):
			...
			logits, Y_prob, Y_hat, A, A_raw = model(features, tile_prob=tile_prob)
	...
	return ids, preds_str, probs, A, A_raw
...
def parse_config_dict(args, config_dict):
	...
	config_dict['model_arguments']['tile_prob']=args.tile_prob
...
#		raise NotImplementedError
	...
	for i in range(len(process_stack)):
		...
		# 	file.close()
		# 	asset_dict = {'attention_scores': A, 'raw_attention_scores':results_dict['A_raw'], 'coords': coords, "tile_probabilities":results_dict['h_probs'], "weighted_tile_probabilities":results_dict['h_weighted']}
			Y_hats, Y_hats_str, Y_probs, A, results_dict = infer_single_slide(model, features, label, reverse_label_dict, exp_args.n_classes, tile_prob=args.tile_prob)
			A_raw = results_dict['A_raw']
			asset_dict = {'A_raw': A_raw, 'coords': coords}
		...
		# HEATMAP_OUTPUT_path = "/rsrch5/home/trans_mol_path/cercan/data/BE/aneuploid/clam/trainings/sep24_progress/1_0/heatmap/16768515_21_s1_fold3_NU_window05/raw/HEATMAP_OUTPUT/"
		...
		# 			scores = dset[:]
		# 			coords = coord_dset[:]
		# 			h_dset = file['tile_probabilities']
		# 			h_dset = h_dset[:]
		# 			hw_dset = file['weighted_tile_probabilities']
		# 			hw_dset = hw_dset[:]
		# 			Aminmax_dset = file['A_minmax']
		# 			Aminmax_dset = Aminmax_dset[:]
		# 			file.close()
		...
		# else:
			# Attention heatmap
					...
					# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_blockmap05window.png'.format('A',slide_id)))

					# # h heatmap
					# # print("h_dset shape")
					# # print(h_dset.shape)
					# heatmap = drawHeatmap(h_dset, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
					# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False, attn_scores = scores)
				
					# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_blockmap05window.png'.format('h',slide_id)))

					# #h weighted heatmap
					# # print("hw_dset shape")
					# # print(hw_dset.shape)
					# heatmap = drawHeatmap(hw_dset, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
					# 				thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False, attn_scores = scores)
				
					# heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_blockmap05window.png'.format('hweighted',slide_id)))

			heatmap = drawHeatmap(A_raw, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
							thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=False)
		
			heatmap.save(os.path.join(r_slide_save_dir, '{}_{}_blockmap.png'.format('A_raw',slide_id)))
