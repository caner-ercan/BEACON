...
#from albumentations import GaussNoise, GaussianBlur, RandomGamma, RandomBrightnessContrast, ToFloat
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        self.hdf5_file = None
        self.hdf5_file_aug = None

    def getOpenSlide(self):
        return self.wsi
    
    def getRGBImage(self, slide_level = None, downsample= 2):
        if slide_level == None:
            slide_level = self.wsi.get_best_level_for_downsample(downsample)
        print(slide_level)
        slide_dim = self.level_dim[slide_level]
        print(slide_dim)
        rgb_img = self.wsi.read_region((0,0), slide_level, slide_dim).convert("RGB")
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
                            filter_params={'a_t':100}, ref_patch_size=512, exclude_ids=[], keep_ids=[], gaus_otsu=False):
        """
        ...
        # Thresholding
        if gaus_otsu:
            #ref https://github.com/manuel-munoz-aguirre/PyHIST/blob/7c2ccb41b97559740d31d5b748f74a89d262252c/src/slide.py#L247
            img_rgb = np.array(self.wsi.read_region((0,0), seg_level, self.level_dim[seg_level]).convert('RGB'))
            img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY)
            img_blur = cv2.GaussianBlur(img_gray,(7,7),0)
            _, img_otsu = cv2.threshold(img_blur, sthresh, sthresh_up, cv2.THRESH_OTSU+cv2.THRESH_BINARY_INV)
        ...
        contours, hierarchy = cv2.findContours(mask_foreground, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE) # Find contours 
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, mode, patch_size=256, step_size=256, custom_downsample=1,
        ...
        count = 0
        save_path_aug=os.path.join(save_path+"_aug")
        name_aug = self.name+"_aug"
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if binarize or attn_scores is not None:
            if thresh < 0:
        ...
        print("rectangle threshold = {:.4f}".format(threshold))
        ...
        for idx in range(len(coords)):
            ...
            if map_any or score >= threshold:
                ...
                #draw rectangle for high attention score patches
                if attn_scores is not None :
                    if attn_scores[idx] >= threshold :
                        thickness=6
                        cv2.rectangle(img, (coord[0]+thickness // 2, coord[1]+thickness // 2),(coord[0]+patch_size[1]-thickness // 2,coord[1]+patch_size[1]-thickness // 2), (0, 0, 0, 255), thickness=thickness)
                        thickness=3
                        cv2.rectangle(img, (coord[0]+thickness // 2, coord[1]+thickness // 2),(coord[0]+patch_size[1]-thickness // 2,coord[1]+patch_size[1]-thickness // 2), (255,255,255, 255), thickness=thickness)
                    
                    # else:
                        # print(attn_scores[idx])
