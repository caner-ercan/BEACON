...
def DrawGrid(img, coord, shape, thickness=4, color=(0,0,0,255)):
...
def StitchPatches(hdf5_file_path, downscale=2, draw_grid=False, bg_color=(0,0,0), alpha=-1, original_image = None):
    ...
    if original_image is None:
        ...
        print("original image is used")
        PIL_original_image = Image.fromarray(original_image)
        if original_image.mode == "RGBA":
            PIL_original_image.putalpha(100)
            heatmap = PIL_original_image
        else:
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=2, draw_grid=False, bg_color=(0,0,0), alpha=-1):
