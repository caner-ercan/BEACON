...
parser.add_argument('-feature_folder', dest='feature_folder', type=str, default=None)
...
for subfolder in folder_list:
    ...
    orig_filenames = list_files_in_folder(orig_data_folder)
    aug_filenames = list_files_in_folder(aug_data_folder)
    ...
    for filename in orig_filenames:
        orig_full_path = os.path.join(orig_data_folder, filename)
        aug_full_path = os.path.join(aug_data_folder, filename)

        if subfolder == "h5":
            # Open the first HDF5 file for reading
            with h5py.File(orig_full_path, 'r') as orig_file:
                orig_features = orig_file['features'][:]  
                orig_coords = orig_file['coords'][:]

            # Open the second HDF5 file for reading
            with h5py.File(aug_full_path, 'r') as aug_file:
